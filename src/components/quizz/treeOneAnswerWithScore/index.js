...
function TreeOneAnswerWithScore(props) {
  ...
  }, [typeFeed, treeQuestion, showFeed]);
  ...
  useEffect(() => {
    if (props.finalFeed) {
      if (showFinalFeed) {
        if (props.setCorrectCounter) {
          props.setCorrectCounter(addZero(correctCounter));
        }
        //seta o feed conforme pontuacao,
        if (correctCounter == 10) {
          //feed perfeito, gabaritou
          setFinalFeedBackItems(props.finalFeed[0]);
        }
        if (correctCounter >= 7 && correctCounter <= 9) {
          //feed positivo
          setFinalFeedBackItems(props.finalFeed[1]);
        }
        if (correctCounter <= 6 && correctCounter >= 0) {
          //feed neutro
          setFinalFeedBackItems(props.finalFeed[2]);
        }
        if (correctCounter <= -1) {
          //feed errado
          setFinalFeedBackItems(props.finalFeed[3]);
        }
  ...
  function validateAnswer(id, cloneAnswers, answered) {
    if (id == checked.indexOf(true)) {
      ...
      answered == "correct" &&
        setCorrectCounter(correctCounter + props.options.scores.correct);
      answered == "neutral" &&
        setCorrectCounter(correctCounter + props.options.scores.neutral);
      answered == "wrong" &&
        setCorrectCounter(correctCounter + props.options.scores.wrong);
...
`}
  ...
  if (!load) {
    ...
    return (
      <Fragment>
        <div className="treeOneAnswerWithScore">
          {questionItems}
          <Row className="justify-content-center">
            <Col>{quizzItems}</Col>
          </Row>
...
export default withScorm()(TreeOneAnswerWithScore);
